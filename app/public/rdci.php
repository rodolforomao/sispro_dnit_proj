...
<div class="container-fluid mt-4">
    <div class="card">
            ...
            <div class="col-md-2">
                ...
                <select name="status" class="form-select" onchange="this.form.submit();">
                    ...
                    <?php foreach ($statusGerais as $s): ?>
                        <option value="<?= htmlspecialchars($s) ?>" <?= ($filtroStatus == $s) ? 'selected' : '' ?>><?= htmlspecialchars($s) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2">
                ...
                <select name="cronograma" class="form-select" onchange="this.form.submit();">
                    ...
                    <?php foreach ($situacoesCronograma as $s): ?>
                        <option value="<?= htmlspecialchars($s) ?>" <?= ($filtroCronograma == $s) ? 'selected' : '' ?>><?= htmlspecialchars($s) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-3">
                <label class="form-label">Buscar</label>
                <input type="text" name="busca" class="form-control" value="<?= htmlspecialchars($filtroBusca) ?>" placeholder="Contrato, obra, empresa..." onchange="this.form.submit();">
            </div>
        ...
        <div class="text-muted small mt-2">Total: <?= count($contratos) ?> contratos</div>
    </div>
</div>
...
function atualizarDados() {
    ...
    const btn = $(this);
    btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat"></i> Atualizando...');
    $.ajax({
        ...
        error: function() {
            ...
            btn.prop('disabled', false).html('<i class="bi bi-arrow-repeat"></i> Atualizar Dados');
        }
    });
}
...
function verDetalhes(id) {
    $.ajax({
        ...
        success: function(data) {
            ...
            let count = 0;
            for (let key in data) {
                if (camposExcluir.includes(key)) continue;
                let valor = data[key] || '—';
